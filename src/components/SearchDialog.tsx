...
export const SearchDialog = ({ labels }: SearchDialogProps) => {
  ...
  const activeHref = flat[active]?.href;

  return (
    <>
      <Button variant="ghost" size="icon" aria-label={labels.open} onClick={openDialog}>
          ...
          >
            ...
            <div id="search-results" role="listbox" className="max-h-[60vh] overflow-y-auto p-2">
              ...
              {!loading && query.length === 0 && (
                <p className="text-muted-foreground px-2 py-6 text-center text-sm">{labels.hint}</p>
              )}
                  ...
                  {items.map((entry) => {
                    ...
                    return (
                      <a
                        ...
                        onClick={close}
                        onMouseEnter={() => setActive(flat.indexOf(entry))}
                        className={cn(
            ...
            <div className="text-muted-foreground hidden items-center gap-4 border-t px-4 py-2 text-xs sm:flex">
              <span>
                <kbd className="bg-muted rounded px-1.5 py-0.5 font-sans">↑↓</kbd> {labels.navigate}
              </span>
              <span>
